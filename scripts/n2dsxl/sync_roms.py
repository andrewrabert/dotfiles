...
ROM_TYPES = [
    ("ds", pathlib.Path("/storage/Games/Nintendo DS"), ".nds"),
    ("gb", pathlib.Path("/storage/Games/Nintendo Game Boy"), ".gb"),
    ("gbc", pathlib.Path("/storage/Games/Nintendo Game Boy Color"), ".gbc"),
    ("gba", pathlib.Path("/storage/Games/Nintendo Game Boy Advance"), ".gba"),
]
...
def get_subdir(name):
    """Get target subdir based on first character of ROM name."""
...
async def sync_roms(
    ...
    for sdcard_subdir, nas_path, extension in rom_types_to_process:
        ...
        nas_roms = set()
        for archive_name, archive_info in index.items():
            if "/" in archive_name:
                continue
            if not archive_name.endswith(archive_ext):
                continue
        ...
        if not path:
            for subdir in sdcard_path.iterdir():
                ...
                for f in subdir.iterdir():
                    if f.suffix == extension and f.name not in nas_roms:
                        unexpected.append(f)
                        print(
                            f"unexpected: {sdcard_subdir}/{subdir.name}/{f.name}"
                        )

        for archive_name, archive_info in index.items():
            # Skip subdirectories (only want top-level files)
            if "/" in archive_name:
                continue

            # Skip non-matching archives
            if not archive_name.endswith(archive_ext):
                continue
            ...
            subdir = get_subdir(rom_name)
            target_path = sdcard_path / subdir / rom_name
