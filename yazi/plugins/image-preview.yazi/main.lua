...
function M:peek(job)
	...
	if
		...
		and (
			file_ext:lower() == "heic"
			or file_ext:lower() == "heif"
			or file_ext:lower() == "tiff"
			or file_ext:lower() == "tif"
		)
	then
		...
		local convert_cmd =
			Command("vips"):arg("jpegsave"):arg(tostring(job.file.url)):arg(temp_file):stderr(Command.PIPED)
