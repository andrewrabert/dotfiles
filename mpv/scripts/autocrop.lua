...
function autocrop_start()
	...
	ret = mp.command(string.format("vf add @%s:cropdetect=limit=%f:round=2:reset=0", cropdetect_label, 24 / 255))
	-- wait to gather data
...
function do_crop()
	-- get the metadata
	local cropdetect_metadata = mp.get_property_native(string.format("vf-metadata/%s", cropdetect_label))
	...
	if cropdetect_metadata then
		...
		then
			mp.command(
				string.format(
					"vf add @%s:lavfi-crop=w=%s:h=%s:x=%s:y=%s",
					crop_label,
					cropdetect_metadata["lavfi.cropdetect.w"],
			...
			mp.msg.error("Got empty crop data. You might need to increase detect_seconds.")
		end
	else
		mp.msg.error("No crop data. Was the cropdetect filter successfully inserted?")
		mp.msg.error("Does your version of ffmpeg/libav support AVFrame metadata?")
	end
